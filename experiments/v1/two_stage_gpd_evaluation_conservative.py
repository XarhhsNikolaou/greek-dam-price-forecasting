...
# --- Adaptive decision threshold (βασισμένο στο hour_extreme_freq) ---
# Απαιτούμενη πιθανότητα classifier για ταξινόμηση ως ακραία ώρα:
#   required = THRESH_MAX - (THRESH_MAX - THRESH_MIN) * hour_extreme_freq
...
def _propagate_to_neighbors(
    ...
    proba_high: np.ndarray, proba_low: np.ndarray, inherit_discount: float = 0.85,
    ...
    for idx in range(len(label)):
        ...
        for neighbor_hour in (hour - 1, hour + 1):
            ...
            if predicted_label[neighbor_idx] != 0:
                ...
                if predicted_label[neighbor_idx] == 2:
...
def run_gpd_evaluation(df: pd.DataFrame, eval_year: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    eval_start = pd.Timestamp(EVAL_START) if EVAL_START else pd.Timestamp(f"{eval_year}-04-01")
    eval_end = pd.Timestamp(EVAL_END) if EVAL_END else pd.Timestamp(f"{eval_year}-04-30")
    ...
    while current_day <= eval_end:
        ...
        hour_freq_test = X_test["hour_extreme_freq"].values
        # Adaptive threshold: χαμηλή ιστορική συχνότητα -> χρειάζεται
        # μεγαλύτερη πιθανότητα classifier για να ταξινομηθεί ως ακραία.
        required_prob = THRESH_MAX - (THRESH_MAX - THRESH_MIN) * hour_freq_test
        ...
        if candidate_hours is not None:
            ...
            if test_cand_mask.sum() > 0:
                proba_cand = classifier.predict_proba(X_test[test_cand_mask])  # στήλες: [0,1,2]
                ...
                # όριο για ΕΚΕΙΝΗ την ώρα (εξαρτάται από hour_extreme_freq).
                cand_idx = np.where(test_cand_mask)[0]
                for j, idx in enumerate(cand_idx):
                    if proba_high[idx] >= required_prob[idx]:
                        predicted_label[idx] = 2
                    elif proba_low[idx] >= required_prob[idx]:
            ...
            predicted_label, proba_high, proba_low = _propagate_to_neighbors(
                predicted_label, df_test["hour"].values, candidate_hours, proba_high, proba_low
            )
        else:
            classifier.fit(X_train, label_train)
            proba_all = classifier.predict_proba(X_test)
        ...
        # --- Confidence-weighted blending: το GPD "βαραίνει" ανάλογα με το
        # πόσο σίγουρος ήταν ο classifier -- χαμηλή σιγουριά (π.χ. ώρα που
        # πήρε την ετικέτα μόνο μέσω διάδοσης) -> πιο κοντά στο ασφαλές
        # normal_pred, περιορίζοντας το κόστος ενός false positive.
        high_sel = predicted_label == 2
        y_pred[high_sel] = (
            proba_high[high_sel] * pred_high_all[high_sel]
            + (1 - proba_high[high_sel]) * pred_normal_all[high_sel]
        )
        low_sel = predicted_label == 1
        y_pred[low_sel] = (
            proba_low[low_sel] * pred_low_all[low_sel]
            + (1 - proba_low[low_sel]) * pred_normal_all[low_sel]
        )
