"""
rolling_evaluation_hour0_original.py
======================================
Ειδικό μοντέλο ΜΟΝΟ για την ώρα 00:00, βασισμένο στο feature set/
hyperparameters του rolling_evaluation_hybrid.py (original -- η ώρα 0
δεν ανήκει στο PEAK_HOURS, άρα ούτως ή άλλως θα έπαιρνε πρόβλεψη από
το γενικό μοντέλο -- εδώ δεν χρειάζεται καν hybrid routing, μόνο ΕΝΑ
μοντέλο).
...
ΓΙΑΤΙ ΕΙΝΑΙ ΑΣΦΑΛΗ ΜΟΝΟ ΓΙΑ ΩΡΑ 0, ΟΧΙ ΓΙΑ ΑΛΛΕΣ ΩΡΕΣ:
  ...
  Άρα γνωστά εκ των προτέρων, καμία διαρροή.

Rolling training: MONTHS_USED (18) πριν από κάθε ημέρα, ΜΟΝΟ σε
δεδομένα ώρας 0.

Eval window: 2025-07-01 -> 2025-08-31 -- τους 2 μήνες καλοκαιριού όπου
αποδίδει καλύτερα το original hybrid (βλ. προηγούμενα πειράματα). Το
"υπόλοιπο" της περιόδου Ιούλιος 2025-Ιούνιος 2026 το καλύπτει το
αντίστοιχο rolling_evaluation_hour0_v2.py.

Οι προβλέψεις γράφονται στο vol2_final.csv, στήλη "hour_specific" --
ΙΔΙΑ στήλη με το rolling_evaluation_hour0_v2.py, αφού καλύπτουν
συμπληρωματικές (όχι επικαλυπτόμενες) περιόδους.
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    (original -- outage_mw μέσα, χωρίς net_load_total_mw) + lag_1h/lag_2h."""
    df = df.copy()
...
if __name__ == "__main__":
    ...
    print("Feature engineering (original base + lag_1h/lag_2h)...")
    ...
    print(f"\nRolling ΩΡΑ-0 evaluation (original hybrid base) {EVAL_START} -> {EVAL_END}...\n")
    ...
    if preds.empty:
        ...
        print("\n" + "=" * 50)
        print(f"ΩΡΑ 00:00 -- MAE/RMSE ({len(preds)} ημέρες, {EVAL_START} -> {EVAL_END}, original hybrid base)")
        print("=" * 50)
        print(f"MAE  = {mae:.2f} EUR/MWh")
        print(f"RMSE = {rmse:.2f} EUR/MWh")

        out_path = DATASET_PATH.parent / "rolling_evaluation_hour0_original_predictions.csv"
    ...
    print(f"  hour_specific: {vol2_final['hour_specific'].notna().sum()} προβλέψεις συνολικά "
          f"(σωρευτικά με το rolling_evaluation_hour0_v2.py, αν έχει ήδη τρέξει)")