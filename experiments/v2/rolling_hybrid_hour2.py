"""
rolling_evaluation_hour2_original.py
======================================
Ειδικό μοντέλο ΜΟΝΟ για την ώρα 02:00, βασισμένο στο feature set/
...
rolling_evaluation_hour2_v2.py.

ΕΞΤΡΑ features lag_1h / lag_2h -- ΚΑΙ ΤΑ ΔΥΟ χρειάζονται πλέον
αντικατάσταση με πρόβλεψη (όχι μόνο το lag_1h όπως στην ώρα 1):
  Για ώρα 2 της ημέρας D:
    lag_1h αναφέρεται σε ώρα 1 της ΙΔΙΑΣ D -- κομμάτι της ίδιας, ακόμα
      άγνωστης δημοπρασίας -> ΧΡΕΙΑΖΕΤΑΙ το μοντέλο ώρας 1.
    lag_2h αναφέρεται σε ώρα 0 της ΙΔΙΑΣ D -- ΕΠΙΣΗΣ κομμάτι της ίδιας
      δημοπρασίας -> ΧΡΕΙΑΖΕΤΑΙ το μοντέλο ώρας 0.
  (Σε αντίθεση με την ώρα 1, όπου το lag_2h ήταν "κανονικό" γιατί
  αναφερόταν στην D-1 -- εδώ δεν υπάρχει καμία "ασφαλής" lag μέσα σε 2
  ώρες, αφού και οι δύο προηγούμενες ώρες ανήκουν στην ίδια D.)

  Στο TEST: lag_1h = πραγματική πρόβλεψη μοντέλου ώρας 1, lag_2h =
    πραγματική πρόβλεψη μοντέλου ώρας 0 (και τα δύο από τη στήλη
    hour_specific του vol2_final.csv, φιλτραρισμένα ανά hour).
  Στο TRAINING: lag_1h = πραγματική τιμή ώρας 1 + θόρυβος std=HOUR1_
    NOISE_STD (RMSE μοντέλου ώρας 1, original = 6.98), lag_2h =
    πραγματική τιμή ώρας 0 + θόρυβος std=HOUR0_NOISE_STD (RMSE μοντέλου
    ώρας 0, original = 7.16).

Στο τέλος συγκρίνει με το "κανονικό" (multi-hour) μοντέλο, ΙΔΙΑ ώρα (2)
ΙΔΙΕΣ ημέρες, μέσω της στήλης predicted_hybrid_original.

Rolling training: MONTHS_USED (18), ΜΟΝΟ δεδομένα ώρας 2.
Eval window: 2025-07-01 -> 2025-08-31.

Οι προβλέψεις γράφονται στο vol2_final.csv, στήλη "hour_specific" (ίδια
στήλη με τα hour0/hour1 scripts -- καμία σύγκρουση, διαφορετικά timestamps).
"""
...
HOUR0_NOISE_STD = 12.04   # RMSE μοντέλου ώρας 0 (original)
HOUR1_NOISE_STD = 10.07   # RMSE μοντέλου ώρας 1 (original)
NOISE_SEED = 42
...
def run_hour2_rolling(df_h2: pd.DataFrame, feature_cols: list,
                       hour0_preds: pd.Series, hour1_preds: pd.Series) -> pd.DataFrame:
    eval_start = pd.Timestamp(EVAL_START)
    eval_end = min(pd.Timestamp(EVAL_END), df_h2["timestamp"].max())
    if pd.Timestamp(EVAL_END) > df_h2["timestamp"].max():
        print(f"[Προσοχή] EVAL_END ({EVAL_END}) > μέγιστη διαθέσιμη ημερομηνία ώρας-2 "
              f"({df_h2['timestamp'].max().date()}). Κόβεται αυτόματα.")

    df_h2 = df_h2.set_index("timestamp").sort_index()
    records = []
    ...
    for i, day in enumerate(test_days, start=1):
        ...
        train = df_h2.loc[train_start:day - pd.Timedelta(seconds=1)]

        test_timestamp = day + pd.Timedelta(hours=2)
        if test_timestamp not in df_h2.index:
        ...
        X_test = df_h2.loc[[test_timestamp], feature_cols].copy()
        X_test["lag_1h"] = hour1_preds.loc[day]
        X_test["lag_2h"] = hour0_preds.loc[day]
        y_test = df_h2.loc[test_timestamp, "mcp_eur_per_mwh"]
...
if __name__ == "__main__":
    ...
    df_h2 = df[df["hour"] == 2].copy()
    print(f"Γραμμές ώρας 02:00 διαθέσιμες: {len(df_h2)}")

    print(f"\nRolling ΩΡΑ-2 evaluation (original base) {EVAL_START} -> {EVAL_END}...\n")
    preds = run_hour2_rolling(df_h2, feature_cols, hour0_preds, hour1_preds)
    ...
    if preds.empty:
        ...
        print("\n" + "=" * 50)
        print(f"ΩΡΑ 02:00 (hour-specific, original base) -- {len(preds)} ημέρες, {EVAL_START} -> {EVAL_END}")
        print("=" * 50)
        ...
        out_path = DATASET_PATH.parent / "rolling_evaluation_hour2_original_predictions.csv"
