...
του rolling_evaluation_hour4_original.py.

ΕΞΤΡΑ features lag_1h / lag_2h -- ΚΑΙ ΤΑ ΔΥΟ χρειάζονται αντικατάσταση
με πρόβλεψη (βλ. αναλυτική αιτιολόγηση στο rolling_evaluation_hour2_
original.py): lag_1h <- μοντέλο ώρας 3, lag_2h <- μοντέλο ώρας 2.

  Στο TEST: πραγματικές προβλέψεις από τη στήλη hour_specific του
    vol2_final.csv (φιλτραρισμένες ανά hour==2 / hour==3).
  Στο TRAINING: πραγματική τιμή + θόρυβος std = RMSE του αντίστοιχου
    μοντέλου (v2 περίοδος): HOUR3_NOISE_STD=15.28, HOUR2_NOISE_STD=14.37.

Στο τέλος συγκρίνει με το "κανονικό" (multi-hour) μοντέλο, ΙΔΙΑ ώρα (4)
ΙΔΙΕΣ ημέρες, μέσω της στήλης predicted_hybrid_v2.
...
HOUR3_NOISE_STD = 15.28   # RMSE μοντέλου ώρας 3 (v2)
NOISE_SEED = 42
...
if __name__ == "__main__":
    ...
    print(f"\nRolling ΩΡΑ-4 evaluation (v2 base) {EVAL_START} -> {EVAL_END}...\n")
    ...
    if preds.empty:
        ...
        print("\n" + "=" * 50)
        print(f"ΩΡΑ 04:00 (hour-specific, v2 base) -- {len(preds)} ημέρες, {EVAL_START} -> {EVAL_END}")
        print("=" * 50)
        ...
        out_path = DATASET_PATH.parent / "rolling_evaluation_hour4_v2_predictions.csv"
