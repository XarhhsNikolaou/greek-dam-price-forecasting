...
hyperparameters του rolling_evaluation_hybrid_v2.py (net_load_total_mw,
χωρίς outage_mw). Καλύπτει 2025-09-01 -> 2026-06-30 -- συμπληρωματικό
του rolling_evaluation_hour3_original.py.

ΕΞΤΡΑ features lag_1h / lag_2h:
  lag_1h <- πρόβλεψη μοντέλου ώρας 2 (test) / πραγματική τιμή + θόρυβος
    std=HOUR2_NOISE_STD=14.37 (training).
  lag_2h <- πρόβλεψη μοντέλου ώρας 1 (test) / πραγματική τιμή + θόρυβος
    std=HOUR1_NOISE_STD=13.42 (training).

Στο τέλος συγκρίνει με το "κανονικό" (multi-hour) μοντέλο, ΙΔΙΑ ώρα (3)
ΙΔΙΕΣ ημέρες, μέσω της στήλης predicted_hybrid_v2 -- ΜΕ ρητό diagnostic
για το πόσες/ποιες μέρες λείπουν από τη στήλη σύγκρισης (π.χ. αν το
rolling_evaluation_hybrid_v2.py δεν έχει ολοκληρώσει ακόμα όλο το
2025-09-01 -> 2026-06-30 στο vol2_final.csv).

Rolling training: MONTHS_USED (18), ΜΟΝΟ δεδομένα ώρας 3.
Eval window: 2025-09-01 -> 2026-06-30.
...
HOUR1_NOISE_STD = 13.42   # RMSE μοντέλου ώρας 1 (v2)
HOUR2_NOISE_STD = 14.37   # RMSE μοντέλου ώρας 2 (v2)
NOISE_SEED = 42

REGULAR_MODEL_COLUMN = "predicted_hybrid_v2"
...
def diagnose_regular_model_coverage(preds: pd.DataFrame, regular: pd.DataFrame, column: str) -> None:
    ...
    if missing:
        print(f"[Προσοχή] Λείπουν {len(missing)} ημέρες από το {column} μέσα στο eval window αυτού "
              ...
              f"σύγκρισης· σημαίνει ότι το rolling_evaluation_hybrid_v2.py δεν έχει (ακόμα) "
              f"προβλέψεις για αυτές τις μέρες στο vol2_final.csv -- ξανατρέξτο (ή αφήστε το να "
              f"ολοκληρωθεί) για ολόκληρο 2025-09-01 -> 2026-06-30.")
...
if __name__ == "__main__":
    ...
    print("Feature engineering (v2 base + lag_1h/lag_2h θορυβώδη)...")
    ...
    print(f"\nRolling ΩΡΑ-3 evaluation (v2 base) {EVAL_START} -> {EVAL_END}...\n")
    ...
    if preds.empty:
        ...
        print("\n" + "=" * 50)
        print(f"ΩΡΑ 03:00 (hour-specific, v2 base) -- {len(preds)} ημέρες, {EVAL_START} -> {EVAL_END}")
        print("=" * 50)
        ...
        out_path = DATASET_PATH.parent / "rolling_evaluation_hour3_v2_predictions.csv"
