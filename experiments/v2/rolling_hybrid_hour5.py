"""
rolling_evaluation_hour5_original.py
======================================
Ειδικό μοντέλο ΜΟΝΟ για την ώρα 05:00, βασισμένο στο feature set/
...
rolling_evaluation_hour5_v2.py.

ΕΞΤΡΑ features lag_1h / lag_2h -- ΚΑΙ ΤΑ ΔΥΟ χρειάζονται πλέον
αντικατάσταση με πρόβλεψη:
  Για ώρα 5 της ημέρας D:
    lag_1h αναφέρεται σε ώρα 4 της ΙΔΙΑΣ D -- κομμάτι της ίδιας, ακόμα
      άγνωστης δημοπρασίας -> ΧΡΕΙΑΖΕΤΑΙ το μοντέλο ώρας 4.
    lag_2h αναφέρεται σε ώρα 3 της ΙΔΙΑΣ D -- ΕΠΙΣΗΣ κομμάτι της ίδιας
      δημοπρασίας -> ΧΡΕΙΑΖΕΤΑΙ το μοντέλο ώρας 3.

  Στο TEST: lag_1h = πραγματική πρόβλεψη μοντέλου ώρας 4, lag_2h =
    πραγματική πρόβλεψη μοντέλου ώρας 3 (και τα δύο από τη στήλη
    hour_specific του vol2_final.csv, φιλτραρισμένα ανά hour).
  Στο TRAINING: lag_1h = πραγματική τιμή ώρας 4 + θόρυβος std=HOUR4_
    NOISE_STD (RMSE μοντέλου ώρας 4, original = 7.83), lag_2h =
    πραγματική τιμή ώρας 3 + θόρυβος std=HOUR3_NOISE_STD (RMSE μοντέλου
    ώρας 3, original = 7.19).

Στο τέλος συγκρίνει με το "κανονικό" (multi-hour) μοντέλο, ΙΔΙΑ ώρα (5)
ΙΔΙΕΣ ημέρες, μέσω της στήλης predicted_hybrid_original.

Rolling training: MONTHS_USED (18), ΜΟΝΟ δεδομένα ώρας 5.
Eval window: 2025-07-01 -> 2025-08-31.

Οι προβλέψεις γράφονται στο vol2_final.csv, στήλη "hour_specific" (ίδια
στήλη με τα hour0/hour1/hour2/hour3/hour4 scripts -- καμία σύγκρουση, διαφορετικά timestamps).
"""
...
HOUR4_NOISE_STD = 7.83   # RMSE μοντέλου ώρας 4 (original)
NOISE_SEED = 42
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    noise1 = rng.normal(loc=0.0, scale=HOUR4_NOISE_STD, size=len(df))
    df["lag_1h"] = lag_1h_actual + noise1

    lag_2h_actual = df["mcp_eur_per_mwh"].shift(2)
    noise2 = rng.normal(loc=0.0, scale=HOUR3_NOISE_STD, size=len(df))
...
def run_hour5_rolling(df_h5: pd.DataFrame, feature_cols: list,
                       hour3_preds: pd.Series, hour4_preds: pd.Series) -> pd.DataFrame:
    eval_start = pd.Timestamp(EVAL_START)
    eval_end = min(pd.Timestamp(EVAL_END), df_h5["timestamp"].max())
    if pd.Timestamp(EVAL_END) > df_h5["timestamp"].max():
        print(f"[Προσοχή] EVAL_END ({EVAL_END}) > μέγιστη διαθέσιμη ημερομηνία ώρας-5 "
              f"({df_h5['timestamp'].max().date()}). Κόβεται αυτόματα.")

    df_h5 = df_h5.set_index("timestamp").sort_index()
    records = []
    ...
    for i, day in enumerate(test_days, start=1):
        ...
        train = df_h5.loc[train_start:day - pd.Timedelta(seconds=1)]

        test_timestamp = day + pd.Timedelta(hours=5)
        if test_timestamp not in df_h5.index:
            print(f"[{i}/{len(test_days)}] {day.date()} -> παραλείπεται (λείπει η ώρα 5)")
            continue
        if len(train) < MIN_TRAIN_DAYS:
            print(f"[{i}/{len(test_days)}] {day.date()} -> παραλείπεται (ελλιπές training, {len(train)} ημέρες)")
            continue
        if day not in hour3_preds.index or day not in hour4_preds.index:
            print(f"[{i}/{len(test_days)}] {day.date()} -> παραλείπεται (λείπει πρόβλεψη ώρας 3 ή 4)")
            continue

        X_train, y_train = train[feature_cols], train["mcp_eur_per_mwh"]

        X_test = df_h5.loc[[test_timestamp], feature_cols].copy()
        X_test["lag_1h"] = hour4_preds.loc[day]
        X_test["lag_2h"] = hour3_preds.loc[day]
        y_test = df_h5.loc[test_timestamp, "mcp_eur_per_mwh"]
        ...
        print(f"[{i}/{len(test_days)}] {day.date()} 05:00 -> actual={y_test:.2f}  "
...
if __name__ == "__main__":
    ...
    print(f"Φόρτωση προβλέψεων ωρών 3 και 4 από: {VOL2_FINAL_PATH}")
    hour3_preds = load_hour_predictions(VOL2_FINAL_PATH, hour=3)
    hour4_preds = load_hour_predictions(VOL2_FINAL_PATH, hour=4)
    print(f"Διαθέσιμες προβλέψεις: ώρα3={len(hour3_preds)}  ώρα4={len(hour4_preds)}")

    df_h5 = df[df["hour"] == 5].copy()
    print(f"Γραμμές ώρας 05:00 διαθέσιμες: {len(df_h5)}")

    print(f"\nRolling ΩΡΑ-5 evaluation (original base) {EVAL_START} -> {EVAL_END}...\n")
    preds = run_hour5_rolling(df_h5, feature_cols, hour3_preds, hour4_preds)
    ...
    if preds.empty:
        ...
        print("\n" + "=" * 50)
        print(f"ΩΡΑ 05:00 (hour-specific, original base) -- {len(preds)} ημέρες, {EVAL_START} -> {EVAL_END}")
        print("=" * 50)
        print(f"MAE  = {mae:.2f} EUR/MWh")
        print(f"RMSE = {rmse:.2f} EUR/MWh")

        regular = load_regular_model_predictions(VOL2_FINAL_PATH, REGULAR_MODEL_COLUMN, hour=5)
        ...
        out_path = DATASET_PATH.parent / "rolling_evaluation_hour5_original_predictions.csv"
